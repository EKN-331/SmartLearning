...
// CLOUD CONFIG
// ============================================================
const CLOUD_API     = 'https://script.google.com/macros/s/AKfycbw3fyl6Ete3aZPFqHsFnYRZUiELg1dfrlFQS-OtsBs37f5M2GE9Bc8Mzo99y4eBUsHB/exec';
...
const App = {
  ...
  async cloudSave(action, payload) {
    ...
    if (!navigator.onLine) {
      ...
      console.log('[Cloud] Offline — queued:', action);
      return;
    }
    ...
    try {
      const res  = await fetch(CLOUD_API, {
        method:  'POST',
        // text/plain menghindari CORS pre-flight pada Apps Script
        headers: { 'Content-Type': 'text/plain;charset=utf-8' },
        body
      });
      const json = await res.json();
      if (!json.ok) console.warn('[Cloud] Save error:', json.error);
      else          console.log('[Cloud] Saved OK:', action, json.data);
    } catch(e) {
      this._syncQueue.push({ action, payload });
      Storage.set('sl_syncQueue', this._syncQueue);
      console.warn('[Cloud] Save failed, queued:', e.message);
    }
  },
  ...
  async _flushSyncQueue() {
    ...
    if (failed.length === 0) {
      console.log('[Cloud] Sync queue flushed ✓');
      this.showToast('☁️ Data offline berhasil disinkronkan!', 'success', 2500);
    } else {
      console.warn('[Cloud] Sync queue — masih ada yang gagal:', failed.length);
    }
  },
  ...
  async _fetchCloudData() {
    ...
    try {
      ...
      const tid = setTimeout(() => controller.abort(), 10000); // timeout 10 detik
      const res  = await fetch(CLOUD_API + '?action=getAll', { signal: controller.signal });
      clearTimeout(tid);
      const json = await res.json();
      ...
      // Tulis ke localStorage hanya jika data cloud tidak kosong
      ...
      // Sinkronkan PIN dari cloud (guru bisa ubah PIN langsung di Sheets)
      ...
      console.warn('[Cloud] Fetch failed:', e.name === 'AbortError' ? 'timeout' : e.message);
      return false;
    }
  },
